...
namespace_ens_begin
Cripple_horizontalNode::Cripple_horizontalNode():m_mesh(nullptr),m_indexVBO(nullptr),
m_dx(5),m_reduceStep(7), m_isDrawDebug(false)
{}

Cripple_horizontalNode::~Cripple_horizontalNode(){
    CC_SAFE_RELEASE_NULL(m_mesh);
    CC_SAFE_RELEASE_NULL(m_indexVBO);
}

Cripple_horizontalNode* Cripple_horizontalNode::create(const string&texFileName)
{
    Cripple_horizontalNode *rippleSprite = new (std::nothrow) Cripple_horizontalNode();
    if (rippleSprite)
    {
        rippleSprite->Sprite::initWithFile(texFileName);
        if (rippleSprite->init()) {
...
Cripple_horizontalNode* Cripple_horizontalNode::createWithSpriteFrameName(const string&texFileName)
{
    Cripple_horizontalNode *rippleSprite = new (std::nothrow) Cripple_horizontalNode();
    if (rippleSprite)
    {
...
bool Cripple_horizontalNode::init(){
