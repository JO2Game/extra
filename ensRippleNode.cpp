...
CrippleSprite::CrippleSprite():m_indexVBO(nullptr),m_mesh(nullptr),m_srcBuffer(nullptr),m_dstBuffer(nullptr),
m_nRow(0),m_nCol(0),m_time(0),m_rippleStrength(8),m_isDrawDebug(false)
{}
CrippleSprite::~CrippleSprite(){
    CC_SAFE_RELEASE_NULL(m_indexVBO);
    CC_SAFE_RELEASE_NULL(m_mesh);
    CC_SAFE_RELEASE_NULL(m_srcBuffer);
    CC_SAFE_RELEASE_NULL(m_dstBuffer);
}

CrippleSprite* CrippleSprite::create(const string&texFileName,float gridSideLen)
{
    CrippleSprite *rippleSprite = new (std::nothrow) CrippleSprite();
    if (rippleSprite)
    {
        rippleSprite->Sprite::initWithFile(texFileName);
        if (rippleSprite->init(gridSideLen)) {
...
CrippleSprite* CrippleSprite::createWithSpriteFrameName(const string&texFileName,float gridSideLen)
{
    CrippleSprite *rippleSprite = new (std::nothrow) CrippleSprite();
    if (rippleSprite)
    {
        rippleSprite->Sprite::initWithSpriteFrameName(texFileName);
        if (rippleSprite->init(gridSideLen)) {
            rippleSprite->autorelease();
            return rippleSprite;
        }
...
bool CrippleSprite::init(float gridSideLen){
