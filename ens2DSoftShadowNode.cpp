...
namespace_ens_begin
ClightNode::ClightNode():m_r(0)
{}

ClightNode* ClightNode::create(const std::string& filename)
{
    ClightNode *sprite = new (std::nothrow) ClightNode();
...
ClightNode* ClightNode::createWithSpriteFrameName(const std::string& spriteFrameName)
{
    ClightNode *sprite = new (std::nothrow) ClightNode();
...
bool ClightNode::initWithFile(const std::string& filename)
{
    bool ret = this->Sprite::initWithFile(filename);
    m_r = min(getContentSize().width, getContentSize().height)*0.4;
    return ret;
}
bool ClightNode::initWithSpriteFrameName(const std::string& spriteFrameName)
{
    bool ret = this->Sprite::initWithSpriteFrameName(spriteFrameName);
    m_r = min(getContentSize().width, getContentSize().height)*0.4;
    return ret;
}
...
C2DSoftShadowSprite::C2DSoftShadowSprite():m_light(nullptr),m_mesh(nullptr),m_finTexture(nullptr),m_indexVBO(nullptr),
m_program(nullptr),m_lightPosLocal(Point::ZERO),m_selfPosLocal(Point::ZERO),
m_shadowLength(1500),m_leftPenumbraPointID(-1),m_rightPenumbraPointID(-1),m_leftUmbraPointID(-1),m_rightUmbraPointID(-1),
m_oneDivObjCount(1.0),m_isDrawDebug(false),m_isDrawNonDebug(true),m_isUpdateShadowSucc(false), m_isDirty(false)
{
...
C2DSoftShadowSprite::~C2DSoftShadowSprite(){
    CC_SAFE_RELEASE_NULL(m_mesh);
    CC_SAFE_RELEASE_NULL(m_finTexture);
    CC_SAFE_RELEASE_NULL(m_indexVBO);
    CC_SAFE_RELEASE_NULL(m_program);
}

C2DSoftShadowSprite* C2DSoftShadowSprite::create(Size size, const std::string& shadowFilename)
{
    C2DSoftShadowSprite *sprite = new (std::nothrow) C2DSoftShadowSprite();
    if (sprite && sprite->initWithFile(size, shadowFilename))
    {
        sprite->autorelease();
        return sprite;
    }
...
bool C2DSoftShadowSprite::initWithFile(Size size, const std::string& shadowFilename)
{
    //bool ret = this->Sprite::initWithFile(filename);
    //this->init(makeRectPolygon(_displaySprite->getContentSize().width*0.4, _displaySprite->getContentSize().height*0.4));
    setShadowImg(shadowFilename);
    //init this sprite
    this->CCSprite::init();
    //setContentSize(size);
    setPolygon(makeRegularPolygon(min(size.width, size.height)*0.4,8));
...
void C2DSoftShadowSprite::setShadowImg(const std::string& filename)
{
    auto tex=CCTextureCache::sharedTextureCache()->addImage(filename);
    if (tex)
    {
        if (m_finTexture)
        {
            m_finTexture->release();
        }
        m_finTexture = tex;
        m_finTexture->retain();
        m_isDirty = true;
...
void C2DSoftShadowSprite::setPolygon(const Cpolygon &polygon)
{
    m_polygon = polygon;
    m_isDirty = true;
}

void C2DSoftShadowSprite::update(float dt){
...
bool C2DSoftShadowSprite::updateShadow(){
    if(m_light==NULL)return false;
    
    Point lightPosLocal=getLightPosLocal();
    Point selfPosLocal = getPosition();
    if (m_lightPosLocal.equals(lightPosLocal) && m_selfPosLocal.equals(selfPosLocal) && m_isDirty == false)
    {
        return true;
    }
    m_lightPosLocal = lightPosLocal;
    m_selfPosLocal = selfPosLocal;
...
void C2DSoftShadowSprite::draw(Renderer* renderer, const Mat4 &transform, uint32_t flags){
    //Sprite::draw(renderer, transform, flags);
    if(m_isDrawNonDebug){
        //this->onDraw(transform, flags);
        _customCommand.init(_globalZOrder);
        _customCommand.func = CC_CALLBACK_0(C2DSoftShadowSprite::onDraw,this,transform, flags);
    ...
    if(m_isDrawDebug){
        ...
        _customCommand_debug.func = CC_CALLBACK_0(C2DSoftShadowSprite::onDrawDebug,this,transform, flags);
        renderer->addCommand(&_customCommand_debug);
    }
    
}
void C2DSoftShadowSprite::onDrawDebug(const Mat4 &transform, uint32_t flags){
...
void C2DSoftShadowSprite::onDraw(const Mat4 &transform, uint32_t flags){
    //----draw mesh
    {
        //----change shader
...
void C2DSoftShadowSprite::setLight(ClightNode*light){
    if(m_light==NULL){
        m_light=light;
        m_light->retain();
    }else{
...
CCPoint C2DSoftShadowSprite::getLightPosLocal(){
...
void C2DSoftShadowSprite::makeFullWindowRectMesh(){
    m_mesh->clear();
...
void C2DSoftShadowSprite::updateMesh(){
    Point lightPosLocal=getLightPosLocal();
    Point selfPosLocal = getPosition();
    if (m_lightPosLocal.equals(lightPosLocal) && m_selfPosLocal.equals(selfPosLocal) && m_isDirty == false)
    {
...
void C2DSoftShadowSprite::submit(GLenum usage){
    //submit mesh
...
//------------------------------------------------------------------------------------------
C2DSoftShadowObj::C2DSoftShadowObj():_displaySprite(nullptr),_shadowSprite(nullptr)
{}
C2DSoftShadowObj::~C2DSoftShadowObj()
{
    CC_SAFE_RELEASE_NULL(_displaySprite);
    CC_SAFE_RELEASE_NULL(_shadowSprite);
}
C2DSoftShadowObj* C2DSoftShadowObj::create(const std::string& filename, const std::string& shadowFilename)
{
    C2DSoftShadowObj *sprite = new (std::nothrow) C2DSoftShadowObj();
    if (sprite && sprite->initWithFile(filename, shadowFilename))
    {
        sprite->autorelease();
        return sprite;
    }
...
C2DSoftShadowObj* C2DSoftShadowObj::createWithSpriteFrameName(const std::string& spriteFrameName, const std::string& shadowFilename)
{
    C2DSoftShadowObj *sprite = new (std::nothrow) C2DSoftShadowObj();
    if (sprite && sprite->initWithSpriteFrameName(spriteFrameName, shadowFilename))
    {
        sprite->autorelease();
        return sprite;
    }
...
bool C2DSoftShadowObj::initWithFile(const std::string& filename, const std::string& shadowFilename)
{
    Sprite *sprite = Sprite::create(filename);
    return init(sprite, shadowFilename);
}
bool C2DSoftShadowObj::initWithSpriteFrameName(const std::string& spriteFrameName, const std::string& shadowFilename)
{
    Sprite *sprite = Sprite::createWithSpriteFrameName(spriteFrameName);
    return init(sprite, shadowFilename);
}
bool C2DSoftShadowObj::init(Sprite *sprite, const std::string& shadowFilename)
{
    _displaySprite = sprite;
    setContentSize(_displaySprite->getContentSize());
    _shadowSprite = C2DSoftShadowSprite::create(_displaySprite->getContentSize(), shadowFilename);
    addChild(_shadowSprite);
    addChild(_displaySprite);
    _displaySprite->retain();
    _shadowSprite->retain();
    _displaySprite->setPosition(Point(getContentSize().width*0.5, getContentSize().height*0.5));
    _shadowSprite->setPosition(Point(getContentSize().width*0.5+_shadowSprite->getContentSize().width*0.5, getContentSize().height*0.5+_shadowSprite->getContentSize().height*.5));
    return true;
}

void C2DSoftShadowObj::setPolygon(const Cpolygon &polygon)
{
    _shadowSprite->setPolygon(polygon);
}
void C2DSoftShadowObj::setLight(ClightNode*light)
{
    _shadowSprite->setLight(light);
}
void C2DSoftShadowObj::setShadowImg(const std::string& filename)
{
    _shadowSprite->setShadowImg(filename);
}
void C2DSoftShadowObj::setOneDivObjCount(float value)
{
    _shadowSprite->setOneDivObjCount(value);
}
float C2DSoftShadowObj::getOneDivObjCount()const
{
    return _shadowSprite->getOneDivObjCount();
}
void C2DSoftShadowObj::setIsDrawDebug(bool value)
{
    _shadowSprite->setIsDrawDebug(value);
}
bool C2DSoftShadowObj::getIsDrawDebug()const
{
    return _shadowSprite->getIsDrawDebug();
}
void C2DSoftShadowObj::setIsDrawNonDebug(bool value)
{
    _shadowSprite->setIsDrawNonDebug(value);
}
bool C2DSoftShadowObj::getIsDrawNonDebug()const
{
    return _shadowSprite->getIsDrawNonDebug();
}

//------------------------------------------------------------------------------------------
C2DSoftShadowRoot::C2DSoftShadowRoot():m_shadowRT(nullptr),m_program(nullptr),m_light(nullptr),m_isDrawDebug(false),m_shadowDarkness(0.5),m_tempSprite(nullptr)
{}

C2DSoftShadowRoot::~C2DSoftShadowRoot(){
    CC_SAFE_RELEASE_NULL(m_shadowRT);
    CC_SAFE_RELEASE_NULL(m_program);
}

C2DSoftShadowRoot* C2DSoftShadowRoot::createShadowRoot(ClightNode* light, Size size)
{
    C2DSoftShadowRoot *sprite = new (std::nothrow) C2DSoftShadowRoot();
    if (sprite && sprite->init(light ,size))
    {
        sprite->autorelease();
        return sprite;
    }
...
bool C2DSoftShadowRoot::init(ClightNode* light, Size size){
    setContentSize(size);
    m_shadowRT=CCRenderTexture::create(size.width, size.height);
    m_shadowRT->retain();
    //----create and set shader program
    {
    ...
    m_tempSprite->setPosition(ccp(size.width/2,size.height/2));
...
void C2DSoftShadowRoot::visit(Renderer* renderer, const Mat4 &parentTransform, uint32_t parentFlags){
        ...
        this->draw(renderer, _modelViewTransform, flags);
    {
        ...
        for(int i=0;i<nObj;i++){
            ...
            obj->getDisplaySprite()->setVisible(false);
            obj->visit();
            //obj->getShadowSprite()->visit();
        ...
        for(int i=0;i<nObj;i++){
            ...
            obj->getDisplaySprite()->setVisible(true);
            obj->visit();
            //obj->getShadowSprite()->visit();
            //obj->getDisplaySprite()->visit();
