...
CshatterSprite::CshatterSprite():m_fragBatchNode(nullptr),m_gridSideLen(2.7f),m_initalFrageScale(4.5f)
{}

CshatterSprite* CshatterSprite::create(const string &texFileName)
{
    CshatterSprite *shatterSprite = new (std::nothrow) CshatterSprite();
    if (shatterSprite && shatterSprite->init(texFileName))
    {
        return shatterSprite;
    }
    CC_SAFE_DELETE(shatterSprite);
    return nullptr;
}

CshatterSprite* CshatterSprite::createWithSpriteFrameName(const string &spriteFrameName)
{
    CshatterSprite *shatterSprite = new (std::nothrow) CshatterSprite();
    if (shatterSprite && shatterSprite->initWithSpriteFrameName(spriteFrameName))
    {
        return shatterSprite;
    }
    CC_SAFE_DELETE(shatterSprite);
...
bool CshatterSprite::initWithSpriteFrameName(const string &spriteFrameName)
{
    CCSprite::initWithSpriteFrameName(spriteFrameName);
    createShatter();
    return true;
}
...
void CshatterSprite::resetShatter(){
    CCPoint offset = Point::ZERO;
    SpriteFrame *frame = getSpriteFrame();
    if (frame)
    {
        offset = frame->getRectInPixels().origin;
    }
    CCSize contentSize=this->getContentSize();
    ...
    for(int i=0;i<nRow;i++){
        for(int j=0;j<nCol;j++){
            ...
            frag->setTextureRect(CCRect(offset.x+x-halfGridSideLen,offset.y+(contentSize.height-y)-halfGridSideLen,m_gridSideLen,m_gridSideLen));
            //set position
...
void CshatterSprite::resetShatterAction()
{
    CCSize contentSize=this->getContentSize();
    int nRow=(int)m_grid.size();
    int nCol=(nRow==0?0:(int)m_grid[0].size());
...
void CshatterSprite::runShatter(float fDuration)
{
    setOpacity(0);
    stopAllActions();
    shatter::CshatterAction*shatter=shatter::CshatterAction::create(fDuration);
    runAction(shatter);
}
//-----------------------------------------
//creates the action
...
void CshatterAction::stop(){
    ...
    //clean up and reset
    ((CshatterSprite*)_target)->resetShatterAction();
	((CshatterSprite*)_target)->m_fragBatchNode->setVisible(false);
    //call father stop
