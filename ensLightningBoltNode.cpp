...
//////////////////////////////---------------------------------
ClightningBoltSprite::ClightningBoltSprite():m_headSeg(nullptr),m_tailSeg(nullptr), m_program(nullptr),m_indexVBO(nullptr),m_mesh(nullptr),
m_mainSegLen(14),m_k_offset(0.13),m_percentageOfJitterBranch(0.5),m_opacityZoom(1.2),m_isDrawDebug(false){
    updateNGeneration();
}
ClightningBoltSprite::~ClightningBoltSprite(){
    CC_SAFE_RELEASE_NULL(m_program);
    CC_SAFE_RELEASE_NULL(m_indexVBO);
    CC_SAFE_RELEASE_NULL(m_mesh);
}

ClightningBoltSprite* ClightningBoltSprite::create(const string&texFileName)
{
    ClightningBoltSprite *boltSprite = new (std::nothrow) ClightningBoltSprite();
    if (boltSprite && boltSprite->Sprite::initWithFile(texFileName))
    {
        boltSprite->init();
        boltSprite->autorelease();
        return boltSprite;
    }
    CC_SAFE_DELETE(boltSprite);
    return nullptr;
}
ClightningBoltSprite* ClightningBoltSprite::createWithSpriteFrameName(const string&texFileName)
{
    ClightningBoltSprite *boltSprite = new (std::nothrow) ClightningBoltSprite();
    if (boltSprite)
    {
        if (boltSprite->Sprite::initWithSpriteFrameName(texFileName))
        {
            boltSprite->init();
            boltSprite->autorelease();
            return boltSprite;
...
void ClightningBoltSprite::initProgram(){
    //init shader program
    {
        ...
        pProgram->attachUniform("color_r");
        pProgram->attachUniform("color_g");
        pProgram->attachUniform("color_b");
        //set program
        m_program=pProgram;
        m_program->retain();
        //check gl error
...
void ClightningBoltSprite::genMesh(){
    ...
    //make mesh
    {
        ...
        for(int i=0;i<nSeg;i++){
            ...
            m_mesh->colorList.push_back(Cv4(color3b.r,color3b.g,color3b.b,opacity));
            m_mesh->colorList.push_back(Cv4(color3b.r,color3b.g,color3b.b,opacity));
            m_mesh->colorList.push_back(Cv4(color3b.r,color3b.g,color3b.b,opacity));
            m_mesh->colorList.push_back(Cv4(color3b.r,color3b.g,color3b.b,opacity));
             */
           // m_mesh->colorList.push_back(Cv4(0.1,0.1,0.1,opacity));
           // m_mesh->colorList.push_back(Cv4(0.3,0.3,0.3,opacity));
           // m_mesh->colorList.push_back(Cv4(0.7,0.7,0.7,opacity));
            //m_mesh->colorList.push_back(Cv4(0.9,0.9,0.9,opacity));
...
void ClightningBoltSprite::onDraw(const Mat4 &transform, uint32_t flags){
    ...
    Color3B color3b = getColor();
    program->passUnifoValue1f("color_r", color3b.r/255.0);
    program->passUnifoValue1f("color_g", color3b.g/255.0);
    program->passUnifoValue1f("color_b", color3b.b/255.0);
    //----enable attributes
...
ClightningBoltNode::ClightningBoltNode():m_lbSprite(nullptr), m_lbSprite2(nullptr), m_isDrawDebug(false)
{}

ClightningBoltNode::~ClightningBoltNode(){}

ClightningBoltNode* ClightningBoltNode::create(const string&texFileName,const CCPoint&start,const CCPoint&end)
{
    ClightningBoltNode *boltNode = new (std::nothrow) ClightningBoltNode();
    if (boltNode && boltNode->initWithFile(texFileName, start, end))
    {
        boltNode->autorelease();
        return boltNode;
    }
    CC_SAFE_DELETE(boltNode);
    return nullptr;
}
ClightningBoltNode* ClightningBoltNode::createWithSpriteFrameName(const string&texFileName,const CCPoint&start,const CCPoint&end)
{
    ClightningBoltNode *boltNode = new (std::nothrow) ClightningBoltNode();
    if (boltNode && boltNode->initWithSpriteFrameName(texFileName, start, end))
    {
        boltNode->autorelease();
        return boltNode;
    }
...
bool ClightningBoltNode::initWithFile(const string&texFileName,const CCPoint&start,const CCPoint&end)
{
    ClightningBoltSprite *sprite1 = ClightningBoltSprite::create(texFileName);
    ClightningBoltSprite *sprite2 = ClightningBoltSprite::create(texFileName);
    return initWithBoltSprite(sprite1, sprite2, start, end);
}
bool ClightningBoltNode::initWithSpriteFrameName(const string&texFileName,const CCPoint&start,const CCPoint&end)
{
    ClightningBoltSprite *sprite1 = ClightningBoltSprite::createWithSpriteFrameName(texFileName);
    ClightningBoltSprite *sprite2 = ClightningBoltSprite::createWithSpriteFrameName(texFileName);
    return initWithBoltSprite(sprite1, sprite2, start, end);
}
bool ClightningBoltNode::initWithBoltSprite(ClightningBoltSprite* sprite1, ClightningBoltSprite* sprite2, const CCPoint&start,const CCPoint&end)
{
    m_start=start;
    m_end=end;
    //lightingSprit
...
void ClightningBoltNode::runFlashAction(float flashTime){
    ...
    //run action
    const float oneFlashTime=flashTime;
