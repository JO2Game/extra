...
CbreakSprite::CbreakSprite():m_mesh(nullptr),m_meshWire(nullptr),m_indexVBO(nullptr),m_indexVBO_wire(nullptr),m_program(nullptr),
m_nRow(0),m_nCol(0),m_gridSideLenMax(80),m_gridSideLenMin(10),m_state(breakEffect::eState_well), m_isDrawDebug(false)
{}

CbreakSprite::~CbreakSprite(){
    CC_SAFE_RELEASE_NULL(m_mesh);
    CC_SAFE_RELEASE_NULL(m_meshWire);
    CC_SAFE_RELEASE_NULL(m_indexVBO);
    CC_SAFE_RELEASE_NULL(m_indexVBO_wire);
    CC_SAFE_RELEASE_NULL(m_program);    
}

CbreakSprite* CbreakSprite::create(const string &texFileName)
{
    CbreakSprite *breakSprite = new (std::nothrow) CbreakSprite();
    if (breakSprite && breakSprite->init(texFileName))
    {
        return breakSprite;
    }
    CC_SAFE_DELETE(breakSprite);
    return nullptr;
}
...
void CbreakSprite::runBreak(float fDuration, float ay)
{
    runAction(breakEffect::CfallOffAction::create(fDuration, ay));
}
//-------------------------------------------------------
//creates the action
